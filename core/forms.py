...
class UploadForm(forms.Form):
    hours_report = forms.FileField(label='Hours Report (Excel)')
    roster = forms.FileField(label='Employee Roster (Excel)')
    assignment = forms.CharField(label='Assignment to Rank', initial='Jeopardy 7a-7a')
    custom_message = forms.CharField(label='Custom Message (Optional)', widget=forms.Textarea, required=False)
